...
> - Pattern recognition 
> - Interpretation
> - Make some ![Image](https://media.acecash.com/~/media/ACE/Images/hand-money-il.ashx?h=476&w=475&la=en&hash=63863FCCBAA5FD68D870EEC6C653211BAC6C27BF) on ![Image](https://www.kaggle.com/static/images/site-logo.png)
...
## First linear model
- Before running the model, we need to split the data first:
...
```{r, message=FALSE, warning=FALSE}
library(glmnet)
x <- model.matrix(data=as.data.frame(listings_big_lm$train), ~ .-price + accommodates*room_type + property_type*neighbourhood_cleansed + review_scores_rating*neighbourhood_cleansed + accommodates*review_scores_rating)	
