...
**Step 2:** Change all the text to lower case.<br>
...
# Take a look at the first 500 characters of the first corpus
substr(corpus[[1]]$content, 1, 500)
...
```{r}
library(modelr)
folds <- crossv_kfold(commentsTrain, k = 5)
folds
folds$test[[1]]
```

```{r}
folds <- folds %>% mutate(model = map(train, ~ rpart(RATING_TEXT ~ . - RATING - LISTING_ID, 
                     data=.,
                     cp=0.001,
                     method="class")))
folds
