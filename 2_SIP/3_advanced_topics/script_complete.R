...
#' We are going to use two distinct data sets. 
#' 
...
#` Filter only listings that are "Downtown"
listings %>% 
...
listings %>% 
    ...
    select(name, review_scores_rating, avg_neighb_rating)
...
#' 2 days. Finally, the output should also have a column for the `price_per_day` of that stay. 
#' 
#' The result should be called `nov_avail` and should look like this: 
#' listing_id    date         price_per_day
#'      24240    2019-11-01             275     *24240 was available 11/01 and 11/02*
#'      24240    2019-11-08             275     *24240 was available 11/08 and 11/09*
#'       ...
#' 
#' 
...
nov_avail = calendar %>% 
    ...
    mutate(price_per_day = (adjusted_price + lead(adjusted_price, 1))/2) %>% 
    filter(available, lead(available, 1)) %>%
    ungroup() %>% 
    filter(
        year(date) == 2019,
        month(date) == 11,
        ...
        2 >= minimum_nights, 
        2 <= maximum_nights) %>%
    select(listing_id, date, price_per_day)
...
#' 
#' 1) Let's start off by creating a data frame of every listing and Friday in November, 
#' and similar one for just the November/December calendar. The reason for this filtering
...
nov_weekends_long = nov_weekends %>%
    inner_join(nov_cal, by = "listing_id") %>%
    mutate(diff_days = as.numeric(difftime(date.y, date.x, units = "days")))

#' - Can you interpret what the join did? 
#' - How is row duplication working to our advantage here?
#' - How many rows does each listing_id/date.x combination have, and what
#'   are the adjusted_price/available columns capturing in those rows?
#' 
#' 3) Now here's the exercise: use a combination of `filter()`, `group_by`, `summarise()` to 
#' get the availability and price of all listings for an `ndays` long stay, where ndays is 
#' a variable you can change the value of. 
#' 
#' The output should be called `nov_avail` as before, but now a row should only be there if: 
#'     - the listing was available for all `ndays` after the Friday
#'     - `ndays` the variable is between the minimum and maximum stays
#' 
...
nov_avail = nov_weekends_long %>%
    filter(diff_days >= 0,
           diff_days < ndays, 
           ndays >= minimum_nights, 
           ndays <= maximum_nights) %>%
    group_by(listing_id, date.x) %>%
    summarise(price_per_day = sum(adjusted_price)/ndays, 
              available_all = all(available)) %>%
    ungroup() %>%
    filter(available_all) %>%
    select(listing_id, date = date.x, price_per_day) 

#' Here's what happened. When we joined on just `listing_id`, there were lots of cross-matches
#' and therefore row duplication. For example, the LHS row (3781, 2019-11-01) matched 30 rows on the RHS, 
#' i.e. all 61 Nov/Dec calendar days for that listing. LHS row (3781, 2019-11-08) also matched those rows, 
#' adding another 61 rows. 
#' We could then treat these sets of 61 rows as groups, filtering to those within the appropriate number of 
#' days of the LHS date and summarising them to get the appropriate prices/availability. 
#' 
...
#' In any case, let's put this into a function to make our life easier later. 
get_availability_table = function(ndays){
    nov_avail = nov_weekends_long %>%
        filter(diff_days >= 0,
               diff_days < ndays, 
               ndays >= minimum_nights, 
...
#' GATHER/SPREAD
#' ----------------------------------------------
#' Let's go back to the slides one last time to learn about long vs. wide data. 
#' 
#' 
#' 
#' Suppose we want to plot, for a given weekend, side-by-side histograms of
#' (a) accomodates (b) bedrooms (c) bathrooms

ndays = 4
npeople = 4
weekend_of = ymd(20191108)

toplot = get_availability_table(ndays) %>%
    filter(date == weekend_of) %>%
    inner_join(creme_de_la_creme, by = c("listing_id" = "id")) %>%
    filter(accommodates >= npeople) %>%
    mutate(price_per_day_person = price_per_day/npeople)
